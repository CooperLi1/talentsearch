...
| People Data Labs | Never used for discovery | Licensed work history, education, skills, and location for an existing candidate | Not implemented | `PEOPLE_DATA_SEARCH_KEY` required | Employment history and education that public technical sources rarely carry |
...
IOI is deliberately not seeded for automatic direct crawling because its `robots.txt` disallows `/results`. An operator can still submit an IOI-style roster URL through the manual deep-dive feature: when direct access is disallowed, the connector may read an existing Exa cached copy with `maxAgeHours: -1`, which explicitly prevents a live crawl, and parse its labeled roster table. If Exa has no cached copy, the run fails with a clear warning instead of bypassing the publisher restriction. ISEF award releases and Breakthrough Junior Challenge are not seeded because their current markup cannot reliably bind a clean person name to one project or award with the generic parser. No generic personal-site or “young builder” list is added automatically.
...
- `PEOPLE_DATA_SEARCH_KEY`: enables licensed person enrichment through the People Data Labs Person Enrichment API. Nothing is fetched from linkedin.com; the provider licenses aggregated public-profile data. Every successful match is billed, so `PDL_DAILY_REQUEST_LIMIT` defaults to 6 across all app instances and the connector performs at most one lookup per person per `lookbackDays` window (default 90 days). Existing LinkedIn URLs use the configured likelihood floor (default 8); fuzzy name-and-affiliation/location recovery requires likelihood 9 or higher and a model review before the result is accepted. Set `PDL_ALLOW_FUZZY_LOOKUPS=false` to disable fuzzy recovery entirely. Only work history, education, skills, location, and the LinkedIn member URL are parsed; contact fields the provider may return are never read or stored.
