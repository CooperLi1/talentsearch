...
- `PEOPLE_DATA_SEARCH_KEY`: enables licensed person enrichment through the People Data Labs Person Enrichment API. Nothing is fetched from linkedin.com; the provider licenses aggregated public-profile data. Every successful match is billed, so the connector performs at most one lookup per person per `lookbackDays` window (default 90 days), requires a configured minimum match likelihood (default 8 of 10) plus a name-correspondence check for fuzzy lookups, and refuses fuzzy lookups without an affiliation or location anchor. Only work history, education, skills, location, and the LinkedIn member URL are parsed; contact fields the provider may return are never read or stored. A LinkedIn URL from a fuzzy match is carried as an unverified identity hypothesis for operator review, never as a verified identity.
