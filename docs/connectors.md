...
- `PEOPLE_DATA_SEARCH_KEY`: enables licensed person enrichment through the People Data Labs Person Enrichment API. Nothing is fetched from linkedin.com; the provider licenses aggregated public-profile data. Every successful match is billed, so `PDL_DAILY_REQUEST_LIMIT` defaults to 3 across all app instances and the connector performs at most one lookup per person per `lookbackDays` window (default 90 days). By default it enriches only an existing LinkedIn member URL. Fuzzy name-and-anchor lookups require the explicit `PDL_ALLOW_FUZZY_LOOKUPS=true` override because the provider bills a successful result before our identity review can discard it. Only work history, education, skills, location, and the LinkedIn member URL are parsed; contact fields the provider may return are never read or stored.
