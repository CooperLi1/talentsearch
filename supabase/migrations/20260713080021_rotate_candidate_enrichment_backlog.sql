alter table public.candidates
  ...
  add column if not exists enrichment_result_count integer not null default 0
    check (enrichment_result_count >= 0);
...
create index if not exists candidates_enrichment_due_idx
...
-- A new policy version makes the stricter readability check visible
-- immediately and prioritizes briefs whose displayed URLs span publishers.
  ...
  with substantive as (
    ...
    where candidates.workspace_id = p_workspace_id
      ...
      and (
        ...
        or candidates.brief_prompt_version is distinct from 'operator-v9'
      )
      and (
