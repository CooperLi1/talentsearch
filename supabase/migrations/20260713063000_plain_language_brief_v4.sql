-- Invalidate v3 briefs when jargon-sensitive rewriting became part of the copy policy.
...
begin
  ...
  with substantive as (
    select
      events.candidate_id,
      md5('operator-v4|' || string_agg(events.content_hash, ',' order by events.discovered_at, events.id)) as fingerprint,
