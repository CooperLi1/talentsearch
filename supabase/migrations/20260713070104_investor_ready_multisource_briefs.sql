-- Regenerate every visible brief under the operator-v7 contract. The contract
-- favors multi-publisher evidence and plain-language, outcome-led bullets.
...
begin
  ...
  with substantive as (
    select events.candidate_id,
      md5('operator-v7|' || string_agg(events.content_hash, ',' order by events.discovered_at, events.id)) as fingerprint,
    ...
    where candidates.workspace_id = p_workspace_id
      ...
      and (
        ...
        or candidates.brief_prompt_version is distinct from 'operator-v7'
      )
