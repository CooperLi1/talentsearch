-- Generate briefs for the full evidence-backed backlog and invalidate older,
-- implementation-heavy operator copy with an explicit prompt version.
...
begin
  ...
  with substantive as (
    select
      events.candidate_id,
      md5('operator-v2|' || string_agg(events.content_hash, ',' order by events.discovered_at, events.id)) as fingerprint,
