-- Invalidate prior copy when the plain-language rewrite became mandatory.
...
begin
  ...
  with substantive as (
    select
      events.candidate_id,
      md5('operator-v5|' || string_agg(events.content_hash, ',' order by events.discovered_at, events.id)) as fingerprint,
