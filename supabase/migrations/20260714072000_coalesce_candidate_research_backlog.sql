-- A useful enrichment pass can insert several evidence rows. Treat those rows
-- as one reason to revisit the candidate, rather than one queue priority point
-- per row, so productive candidates cannot starve first-pass research.
...
as $$
  ...
  set research_input_revision = greatest(
        research_input_revision,
        research_completed_revision + 1
      ),
...
begin
  ...
  set enrichment_attempt_count = next_attempt_count,
      ...
      -- Coalesce every event inserted during this pass into one completed
      -- research revision. A fresh pass is still scheduled below.
...
-- Collapse the legacy per-event gaps to one pending follow-up per candidate.
update public.candidates
set research_input_revision = research_completed_revision + 1
where status <> 'archived'
  and research_input_revision > research_completed_revision + 1;
