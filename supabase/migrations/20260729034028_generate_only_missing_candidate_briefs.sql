-- Prompt and model identifiers are provenance, not automatic invalidation.
-- Claim only candidates that have never received a brief. Any future backfill
-- must be an explicit, separately budgeted operation.
...
comment on function public.claim_candidate_brief_batch(bigint, integer, integer) is
  'Claims only candidates that have never received a generated brief.';
