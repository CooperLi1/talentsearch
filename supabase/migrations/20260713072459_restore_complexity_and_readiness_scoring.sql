-- Re-score and regenerate briefs after restoring repository complexity metrics
-- from persisted evidence and reducing quantity-only project inflation.
...
begin
  ...
  with substantive as (
    select events.candidate_id,
      md5('operator-v8|' || string_agg(events.content_hash, ',' order by events.discovered_at, events.id)) as fingerprint,
    ...
    where candidates.workspace_id = p_workspace_id
      ...
      and (
        ...
        or candidates.brief_prompt_version is distinct from 'operator-v8'
      )
