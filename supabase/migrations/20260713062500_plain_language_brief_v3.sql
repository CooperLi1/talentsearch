-- Rewrite stored briefs under the shorter, nontechnical operator-copy policy.
...
begin
  ...
  with substantive as (
    select
      events.candidate_id,
      md5('operator-v3|' || string_agg(events.content_hash, ',' order by events.discovered_at, events.id)) as fingerprint,
