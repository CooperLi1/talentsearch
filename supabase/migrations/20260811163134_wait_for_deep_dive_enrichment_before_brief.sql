-- A manual deep dive is an operator request for research, not just ingestion.
-- Do not generate its first brief until the research revision created by that
-- deep dive has either completed or been durably deferred by a provider reset.
...
begin
  ...
  with substantive as (
    ...
    where candidates.workspace_id = p_workspace_id
      ...
      and candidates.brief_generated_at is null
      and exists (
        select 1
...
comment on function public.claim_candidate_brief_batch(bigint, integer, integer) is
  'Claims missing briefs after any operator-requested deep-dive research revision completes.';
